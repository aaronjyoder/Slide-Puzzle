...
public class Puzzle extends JFrame {
    ...
    MouseListener ml = new MouseAdapter() {
      ...
      public void mouseReleased(MouseEvent e) {
        ...
        if (currentPiece != null) {
          ...
          if (blankPiece != null) {
            movePiece(currentPiece, blankPiece.getPosition().x, blankPiece.getPosition().y);
            rebuildPanel();
          }
        }
  ...
  private void movePiece(Piece p, int x, int y) {
    int oldX = p.getPosition().x;
    int oldY = p.getPosition().y;
  ...
  private void generatePieces() {
    ...
    for (int x = 0; x < size; x++) {
      for (int y = 0; y < size; y++) {
        pieces[x][y] = new Piece(resizeImage(image.getSubimage(x * pieceSize, y * pieceSize, pieceSize, pieceSize), 1), new Point(x, y));
      }
    }
